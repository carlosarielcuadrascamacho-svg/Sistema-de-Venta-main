...
const productosDePrueba = [
  ...
  {
    nombre: "Unidad SSD 1TB Kingston",
    precio: 2100,
    stock: 30,
    categoria: "Componentes",
    imagen: "ssd.png",
  ...
  try {
    ...
    for (let emp of empDb) {
      nominas.push({
        empleado: emp._id,
        monto: emp.salario,
        fecha_pago: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000),
      }); // Hace 2 meses
      nominas.push({
        empleado: emp._id,
        monto: emp.salario,
        fecha_pago: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
      }); // Hace 1 mes
      nominas.push({
        empleado: emp._id,
        monto: emp.salario + 500,
        fecha_pago: new Date(),
      }); // Hoy (con bono)
    }
    ...
    console.log("--------------------------------------------------");
    console.log(
      `📊 Se inyectaron: ${prodsDb.length} productos, ${empDb.length} empleados, ${nominas.length} pagos y ${ventas.length} ventas de historial.`,
    );
