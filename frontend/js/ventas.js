...
function agregarAlCarrito() {
  ...
  if (cantidad > productoInfo.stock) {
    alert(
      `Solo hay ${productoInfo.stock} unidades de ${productoInfo.nombre} disponibles.`,
    );
    return;
  }
...
async function procesarVenta() {
  ...
  try {
    ...
    const productosParaBackend = carrito.map((item) => ({
      // Asegúrate de que el backend espera "producto" y no "idProducto" según lo tengas configurado
    ...
    // Si fetchAPI no saltó al 'catch', la venta fue un éxito
    alert("Venta registrada con éxito");

    // 1. Limpiamos el carrito y actualizamos la tabla
    carrito = [];
    renderizarCarrito();

    // 2. Recargamos los productos para que el select muestre el nuevo stock desde la BD
    await cargarProductosParaVenta();

    // 3. Actualizamos el total de ventas de hoy en la pantalla
    actualizarTotalDashboard();
  } catch (error) {
    alert("Error al procesar la venta: " + error.message);
  }
}

// Nueva función para refrescar solo el dato del dashboard (definida por fuera)
async function actualizarTotalDashboard() {
  try {
    const response = await fetch("/api/ventas/hoy"); // Ajusta a tu endpoint real si es diferente
    if (!response.ok) return; // Si hay error silencioso, no hacemos nada

    const data = await response.json();

    // Validamos que el elemento HTML exista en esta pantalla antes de intentar cambiarlo
    const elementoTotal = document.getElementById("total-ventas-hoy");
    if (elementoTotal) {
      elementoTotal.textContent = `$${data.total}`;
    }
  } catch (error) {
