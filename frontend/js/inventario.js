...
async function cargarInventario() {
  try {
    ...
    productos.forEach((prod) => {
      ...
      const stockStyle =
        prod.stock <= 5 ? "color: red; font-weight: bold;" : "";
      ...
      tr.innerHTML = `
          ...
          <td>
              <button class="btn btn-outline-success btn-sm" onclick="agregarStock('${prod._id}')">➕ Stock</button>
...
// Función para pedir la cantidad y enviarla al backend
async function agregarStock(idProducto) {
  const cantidad = prompt("¿Cuántas unidades deseas agregar al inventario?");

  // Validamos que el usuario haya escrito un número válido y mayor a 0
  if (!cantidad || isNaN(cantidad) || parseInt(cantidad) <= 0) return;

  try {
    // CORRECCIÓN 3: Usamos fetchAPI para mantener la consistencia del archivo
    await fetchAPI(`/productos/${idProducto}/stock`, {
      method: "PUT",
      body: JSON.stringify({ cantidad: parseInt(cantidad) }),
    });

    alert("Stock actualizado correctamente");
    // CORRECCIÓN 2: Llamamos a la función correcta para refrescar la tabla
    cargarInventario();
  } catch (error) {
