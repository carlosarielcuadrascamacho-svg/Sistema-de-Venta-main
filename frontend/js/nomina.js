...
async function cargarDatosRH() {
  ...
  // Activamos la calculadora automática justo cuando se carga la vista
  iniciarAutoCalculo();
}

// Función exclusiva para escuchar las teclas y autocalcular
function iniciarAutoCalculo() {
  const inputHoras = document.getElementById("horas-trabajadas");
  const inputPago = document.getElementById("pago-hora");
  // Usamos 'pago-monto' que es el ID que tu formulario envía al backend
  const inputTotal = document.getElementById("pago-monto");

  function autoCalcularSalario() {
    const horas = parseFloat(inputHoras.value) || 0;
    const pago = parseFloat(inputPago.value) || 0;

    // Multiplicamos y forzamos 2 decimales
    if (inputTotal) {
      inputTotal.value = (horas * pago).toFixed(2);
    }
  }

  // Escuchamos cada vez que el usuario escriba algo en horas o pago
  if (inputHoras && inputPago) {
    inputHoras.addEventListener("input", autoCalcularSalario);
    inputPago.addEventListener("input", autoCalcularSalario);
  }
}
...
document.getElementById("form-pago").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // Limpiamos también los campos de horas para el siguiente pago
    document.getElementById("horas-trabajadas").value = "";
    document.getElementById("pago-hora").value = "";
